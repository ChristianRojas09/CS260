...
Bid BinarySearchTree::Search(string bidId) {
	...
	Node* current = root; //start searching for the root
...
Node* BinarySearchTree::removeNode(Node* node, string bidId) {
	...
	if(bidId.compare(node->bid.bidId) < 0) {
		...
		else if(node->left != nullptr && node->right == nullptr) { //left child
			Node* temp = node;
			node = node->left;
			delete temp;
		}
